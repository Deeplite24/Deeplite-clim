    ...
    function getSeenGroupJoins() {
      ...
      try { return JSON.parse(localStorage.getItem('deeplite_seen_group_joins_' + currentUid) || '[]'); } catch (e) { return []; }
    }
    function markGroupJoinSeen(groupId) {
      if (!currentUid) return;
      const list = getSeenGroupJoins();
      if (!list.includes(groupId)) { list.push(groupId); localStorage.setItem('deeplite_seen_group_joins_' + currentUid, JSON.stringify(list)); }
    }
    ...
    // ⚠️ هاد الدالتين (togglePinItem وpinBtnHtml) كانو ناقصين تماماً من هاد الملف رغم أن
    // data.js كيستدعي pinBtnHtml() فـ4 بلايص (شيكات، مخزون، تركيب، ملاحظات) — هادشي كان
    // كيخرب رندر القوائم الأربعة كاملة (crash فوسط الـ.map()، القائمة تبقى فارغة/ما كتبانش).
